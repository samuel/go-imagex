...
func average(sum []uint64, w, h int, n uint64) image.Image {
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			index := 4 * (y*w + x)
			ret.Pix[y*ret.Stride+x*4] = uint8(sum[index+0] / n)
			ret.Pix[y*ret.Stride+x*4+1] = uint8(sum[index+1] / n)
			ret.Pix[y*ret.Stride+x*4+2] = uint8(sum[index+2] / n)
			ret.Pix[y*ret.Stride+x*4+3] = uint8(sum[index+3] / n)
		}
	}
	return ret
}
...
func resizeRGBA(m *image.RGBA, r image.Rectangle, w, h int) image.Image {
	...
	for y := r.Min.Y; y < r.Max.Y; y++ {
		...
		for x := r.Min.X; x < r.Max.X; x++ {
			// Get the source pixel.
			r64 := uint64(pix[4*x])
			g64 := uint64(pix[4*x+1])
			b64 := uint64(pix[4*x+2])
			a64 := uint64(pix[4*x+3])
			// Spread the source pixel over 1 or more destination rows.
			py := uint64(y-r.Min.Y) * hh
			for remy := hh; remy > 0; {
				qy := dy - (py % dy)
